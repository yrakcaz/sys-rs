...
/// * `pid` - The process ID of the pid process.
...
pub fn trace_with(
    ...
    let pid = process.pid();
    let mut breakpoint_mgr = breakpoint::Manager::new(pid);

    let mut startup_complete = false;
    let mut state = progress::State::new(pid, src_available);
                ...
                WaitStatus::Stopped(_, Signal::SIGTRAP) => {
                    ...
                    if let Some(opcode) = process.get_opcode_from_addr(rip)? {
                        ...
                        if instruction.is_call() {
                            ...
                            let ret_addr =
                                ptrace::read(pid, regs.rsp as ptrace::AddressType)?
                                    as u64;
